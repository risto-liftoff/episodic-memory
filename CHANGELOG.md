...
### Fixed
- **Claude Code MCP startup**: Add npm `overrides` to force `mime-types@3.0.1`, working around upstream dependencies requesting `mime-types@^3.0.2` (which currently does not exist on the public registry).

### Security
- **MCP file read restriction**: MCP `read` tool now only allows reading `.jsonl` files within the conversation archive directory (prevents arbitrary file reads / symlink escapes).
